...
Function_gte::Function_gte(const Function_gte& function)
:Function(function)
{
  try
  {
...
float Function_gte::executeFunctionCall1(std::vector<float>& parameters)
{
  try
  {
...
double Function_gte::executeFunctionCall1(std::vector<double>& parameters)
{
  try
  {
    if (parameters.size() == 2)
...
void Function_gte::executeFunctionCall9(uint columns,uint rows,std::vector<std::vector<float>>& inParameters,const std::vector<double>& extParameters,std::vector<float>& outParameters)
{
  try
  {
...
void Function_gte::executeFunctionCall9(uint columns,uint rows,std::vector<std::vector<double>>& inParameters,const std::vector<double>& extParameters,std::vector<double>& outParameters)
{
  try
  {
    uint sz = columns*rows;
...
Function* Function_gte::duplicate()
{
  try
  {
    return (Function*)new Function_gte(*this);
  }
  catch (...)
  {
    throw Fmi::Exception(BCP, "Operation failed!", nullptr);

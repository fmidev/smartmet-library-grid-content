...
void Server::init(DataServer::ServiceInterface *serviceInterface)
{
  try
  {
    ...
    try
    {
      PortableServer::ObjectId_var service_oid = PortableServer::string_to_ObjectId("DataService");
      mPoa->activate_object_with_id(service_oid, &mCorbaInterface);

      CORBA::Object_var service_obj = ((DataServer::Corba::ServerInterface*)&mCorbaInterface)->_this();

...
void Server::init(ContentServer::ServiceInterface *serviceInterface)
{
  try
  {
    ...
    try
    {
      PortableServer::ObjectId_var service_oid = PortableServer::string_to_ObjectId("ContentService");
      mPoa->activate_object_with_id(service_oid, &mCorbaInterface);

      CORBA::Object_var service_obj = ((ContentServer::Corba::ServerInterface*)&mCorbaInterface)->_this();
...
uint Server::getServerId()
{
  ...
  {
    char tmp[100];
    sprintf(tmp,"%s:%s",mAddress.c_str(),mPort.c_str());
    return stringToId(tmp,20);
  }
  catch (...)
  {
    throw Fmi::Exception(BCP,"Operation failed!",nullptr);
